...
class BinaryCategorizationTaskFFWD(Task):
    ...
    def sample(self):
        ...
        example_input         = resp
        example_output        = np.zeros((self.batch_size, self.n_out))
        example_output[:,:]   = c
        r                     = np.squeeze(resp)
        a                     = np.ones(shape=(self.n_in, )) / self.sigma_sq
        e                     = self.phi / self.sigma_sq
...
class CausalInferenceTask(Task):
    ...
    def sample(self):
        ...
        s1e = S[:,:,:int(0.8*self.n_in)]
        s1i = S[:,:,int(0.8*self.n_in):self.n_in]
        s2e = S[:,:,self.n_in:int(1.8*self.n_in)]
        s2i = S[:,:,int(1.8*self.n_in):]
        ...
        R1e = R[:,:,:int(0.8*self.n_in)]
        R2e = R[:,:,int(0.8*self.n_in):int(1.6*self.n_in)]
        R1i = R[:,:,int(1.6*self.n_in):int(1.8*self.n_in)]
        R2i = R[:,:,int(1.8*self.n_in):]
