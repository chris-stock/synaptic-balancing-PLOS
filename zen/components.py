...
def upper_triang_initializer(mean=0.1, stddev=1.0):
    """Generates a function for random Gaussian initialization
    with only upper diagonal elements
    """
    def _init(shape, i):
        mat = np.random.randn(*shape).astype(np.float32)*stddev + mean
        return np.triu(mat)
    return _init
...
def glorot_initializer(scale=1.0):
    def _init(shape, i):
        return (scale * np.sqrt(2) / np.sqrt(np.mean(shape))) * np.random.randn(*shape).astype(np.float32)
    return _init
...
def custom_initializer(W_init, *args, **kwargs):
    ...
    def _init(shape, i):
        try:
            ...
            raise IndexError('W_init must have as many elements as there are weight matrices')
        assert W_i.shape == shape, 'W_init[{}] has shape {}, should have shape {}'.format(i, W_init.shape, shape)
